  ...
  return new Promise((resolve, reject) => {
    ...
    if (isDev) {
      ...
      const execPath = isWindows
        ? path.join(process.resourcesPath, 'aiagent', 'suggestor.exe')
        : path.join(process.resourcesPath, 'aiagent', 'suggestor');
...
function waitForCerebroReady(timeout = 90000, interval = 1000) {
  const deadline = Date.now() + timeout;

  return new Promise((resolve, reject) => {
    const check = () => {
      const req = http.get({ hostname: '127.0.0.1', port: 8000, path: '/', timeout: 1500 }, (res) => {
        res.destroy();
        resolve(true);
      });

      const retry = () => {
        if (Date.now() > deadline) return reject(new Error('Timed out waiting for Cerebro on 127.0.0.1:8000'));
        setTimeout(check, interval);
      };
...
app.whenReady().then(() => {
  ...
  const isWindows = process.platform === 'win32';
  const cerebroPath = isDev
    ? path.join(__dirname, 'agent_build', isWindows ? 'cerebro.exe' : 'cerebro')
    : path.join(process.resourcesPath, isWindows ? 'cerebro.exe' : 'cerebro');

  cerebroProcess = spawn(cerebroPath, [], {
    cwd: path.dirname(cerebroPath),
    env: {
      ...process.env,
      PYTHONUTF8: '1',
    },
  ...
  cerebroProcess.on('error', err => {
    console.error('❌ Cerebro process failed to start:', err);
  });
...
app.on('window-all-closed', () => {
  ...
  if (cerebroProcess && !cerebroProcess.killed) {
    kill(cerebroProcess.pid, 'SIGKILL', (err) => {
      if (err) console.error('❌ Failed to kill cerebro:', err);
