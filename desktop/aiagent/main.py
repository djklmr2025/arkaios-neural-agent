...
def perform_action(response):
    ...
    for action in actions:
        try:
            ...
            # SAFETY GUARD CHECK
            if not safety_guard.check_action_safety(act, params):
                print(f"⛔ Acción bloqueada por Safety Guard: {act} {params}")
                # Enviar reporte de fallo al backend si se desea (en este caso solo salta la acción)
                continue
...
def get_next_step():
    ...
    if not api_url or not thread_id:
        time.sleep(2)  # Wait and don't spam if not configured
        return None

    url = f"{api_url}/aiagent/{thread_id}/next_step"
    headers = {
...
def get_current_subtask():
    ...
    url = f"{api_url}/aiagent/{thread_id}/current_subtask"
    headers = {
        'Content-Type': 'application/json',
        'Authorization': f'Bearer {access_token}',
    }
